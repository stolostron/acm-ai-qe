...
| 3 | Classification Breakdown | `summary.by_classification` (sorted by count descending), `summary.cascading_hook_failures` (v3.2), `summary.blank_page_failures` (v3.2) |
...
| 8 | Cluster Investigation (v3.0) | `analysis_results.cluster_investigation` — degraded operators, pod diagnostics |
| 9 | Feature Knowledge & Cluster Investigation (v3.1) | `cluster_investigation_summary` (Tier 0 health, prerequisites, component health, playbook investigations), `cluster_access` status |
| 10 | Feature Area Grouping (v3.0) | `raw_data.feature_grounding.groups` — tests grouped by feature area with subsystem context |
| 11 | Environment Status | `environment.cluster_connectivity`, `environment.api_accessibility`, `environment.environment_score` |
| 12 | Feedback (v3.0) | CLI instructions for `feedback.py` — rate classifications for accuracy tracking |
