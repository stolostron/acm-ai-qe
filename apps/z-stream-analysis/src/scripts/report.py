...
class ReportFormatter:
        ...
        try:
            ...
            if not result.is_valid:
                message = (
                    ...
                    f"See output example: .claude/agents/z-stream-analysis.md (search for 'Output Schema')\n\n"
    ...
    def format_markdown(self) -> Path:
        ...
        if self.analysis_results:
            ...
            if by_classification:
                ...
                # Show cascading hook failures and blank page failures if present
                cascading_hooks = overall.get('cascading_hook_failures', 0)
                blank_pages = overall.get('blank_page_failures', 0)
                if cascading_hooks > 0 or blank_pages > 0:
                    lines.append("")
                    if cascading_hooks > 0:
                        lines.append(f"- *Cascading hook failures (NO_BUG):* {cascading_hooks}")
                    if blank_pages > 0:
                        lines.append(f"- *Blank page failures (prerequisite missing):* {blank_pages}")
        ...
        if self.analysis_results:
            cluster_inv = self.analysis_results.get('cluster_investigation', {})
            if cluster_inv:
                lines.extend(self._format_cluster_investigation(cluster_inv))

        # Feature Knowledge & Cluster Investigation (v3.0)
        if self.analysis_results:
            lines.extend(self._format_feature_knowledge_section())

        # Feature Area Grouping (v3.0)
        feature_grounding = self.raw_data.get('feature_grounding', {})
        if feature_grounding and not feature_grounding.get('error'):
            groups = feature_grounding.get('groups', {})
            if groups:
                lines.extend([
                    "---",
                    "",
                ...
                for area, group_data in groups.items():
                    subsystem = group_data.get('subsystem', 'Unknown')
                    test_count = group_data.get('test_count', 0)
                    components = group_data.get('key_components', [])
                    lines.append(f"- **{area}** ({subsystem}): {test_count} test(s)")
                    if components:
        ...
        # Feedback section (v3.0)
        lines.extend([
            "---",
            "",
            "## Feedback",
            "",
            "Rate this analysis to improve classification accuracy over time:",
            "",
            "```bash",
            "# Mark a test classification as correct",
            f"python -m src.scripts.feedback {self.run_dir} --test \"<test_name>\" --correct",
            "",
            "# Mark as incorrect with correction",
            f"python -m src.scripts.feedback {self.run_dir} --test \"<test_name>\" --incorrect \\",
            "    --should-be PRODUCT_BUG --note \"reason\"",
            "",
            "# View accuracy stats",
        ...
        lines.extend([
            "---",
            "",
            "*Report generated by Z-Stream Analysis Framework v3.2*",
    ...
    def _format_cluster_investigation(self, cluster_inv: Dict[str, Any]) -> List[str]:
        """Format cluster investigation section from AI results."""
        lines = [
            "---",
            "",
        ...
        # Landscape
        landscape = cluster_inv.get('landscape', {})
        if landscape:
            degraded = landscape.get('degraded_operators', [])
            pressure = landscape.get('resource_pressure', {})
            mch_status = landscape.get('multiclusterhub_status', 'Unknown')

            lines.extend([
                f"- **Managed Clusters:** {landscape.get('managed_cluster_count', 'N/A')}",
                f"- **MCH Status:** {mch_status}",
            ])
            ...
            if active_pressure:
                ...
                lines.append("- **Resource Pressure:** None")
    ...
    def _format_feature_knowledge_section(self) -> List[str]:
        """Format Feature Knowledge & Cluster Investigation section from AI results."""
        if not self.analysis_results:
            return []

        # Check for feature_knowledge in raw data and cluster_investigation_summary in analysis
        feature_knowledge = self.raw_data.get('feature_knowledge', {})
        cluster_inv_summary = self.analysis_results.get('cluster_investigation_summary', {})

        if not feature_knowledge and not cluster_inv_summary:
            return []

        lines = [
            "---",
            "",
            "## Feature Knowledge & Cluster Investigation",
            "",
        ]

        # Cluster Access mode
        if cluster_inv_summary:
            reauth = cluster_inv_summary.get('cluster_reauth_status', 'unknown')
            mode = cluster_inv_summary.get('investigation_mode', 'unknown')
            mode_label = 'Live (re-authenticated)' if mode == 'live' else 'Degraded (snapshot only)'
            lines.extend([
                f"- **Cluster Access:** {mode_label} (re-auth: {reauth})",
                "",
            ])
        ...
        playbook_tests = [
            t for t in per_test
            if t.get('playbook_investigation') or t.get('prerequisite_analysis')
        ]
        if playbook_tests:
            lines.extend([
                "### Playbook Investigations",
                "",
                "| Test | Failure Path | Tier | Confirmed | Classification |",
                "|------|-------------|------|-----------|----------------|",
            ])
            for t in playbook_tests:
    ...
    def format_summary(self) -> Path:
        ...
        if self.analysis_results:
            ...
            if by_classification:
                ...
                blank_pages = overall.get('blank_page_failures', 0)
                if cascading_hooks > 0:
                    lines.append(f"  (includes {cascading_hooks} cascading hook failure(s))")
                if blank_pages > 0:
                    lines.append(f"  (includes {blank_pages} blank page failure(s))")
