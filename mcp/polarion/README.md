...
```json
{
  "polarion": {
    ...
    "args": ["--with", "polarion-mcp", "python",
             "<repo-root>/mcp/polarion/polarion-mcp-wrapper.py"],
    "cwd": "~",
    "env": {
...
| `.cursorrules` (workspace root) | AI rules referencing Polarion MCP (query syntax, defaults, behaviors) |
...
| `tools/polarion/` (automation workspace) | Older standalone CLI tool (not this MCP) |
