...
[Read from phase1-jira.json. JSON values are authoritative for: acceptance_criteria, linked_tickets, pr_references]

--- CODE CHANGE ANALYSIS (source: phase2-code.json) ---
[Read from phase2-code.json. JSON values are authoritative for: field_orders, filter_functions (exact conditions), coverage_gaps]

--- UI DISCOVERY (source: phase3-ui.json) ---
[Read from phase3-ui.json. JSON values are authoritative for: routes, translations_verified, selectors, entry_point]
...
- JSON file authority: when phase JSON files (`phase1-jira.json`, `phase2-code.json`, `phase3-ui.json`) exist in the run directory, their structured values (filter conditions, field order arrays, route paths, AC lists) take precedence over conversation text for the same data point
```
...
#### Annotation-Based Conditional Rendering

When code analysis shows UI elements that are conditional on Kubernetes annotations, ConfigMap values, or CR fields (e.g., `grafanaLink && (<AcmButton...>)`), these ARE in scope for UI test cases. The CLI command to toggle the annotation IS the test action. Do NOT exclude these as "cluster-level manipulation" -- they are how the user controls the conditional UI behavior. Each independent annotation-based conditional requires its own negative scenario step.
...
2. **Read the area knowledge file** (`.claude/knowledge/ui/<area>.md`) for documented navigation paths to that component.
