...
- **Tampered console warning** — when the console is running a non-official image, `console_search.found` results in core-data.json are unreliable for classification. The agent must verify selectors against the official source via ACM Source MCP before attributing failures to the tampered image.
