...
**Output:** Four report files in the run directory

```
analysis-results.json ──┐
                        ├──► ReportFormatter ──┬── Detailed-Analysis.md
core-data.json ─────────┘                     ├── analysis-report.html (interactive)
...
**Required fields in analysis-results.json:** `per_test_analysis` (NOT `failed_tests`), `summary.by_classification`, `investigation_phases_completed`. See `src/schemas/analysis_results_schema.json` for the full schema.
...
| Invalid (strict mode, default) | Raises `ValueError`, stops report generation |
| `per_test_analysis` missing or empty | Raises `ValueError` (common mistake: using `failed_tests` instead) |
...
│     └── manifest.json? → core-data.json (multi-file)    │
│         raw-data.json? → migration check                │
│         else           → core-data.json (direct)        │
│                                                         │
│  2. _load_json('analysis-results.json') → self.analysis │
│                                                         │
│  3. _validate_analysis_results(strict=True)              │
│     └── SchemaValidationService validates against       │
│         src/schemas/analysis_results_schema.json        │
│         (strict mode — raises ValueError on errors)     │
└───────────────────────┬─────────────────────────────────┘
                        │
                        ▼
┌─────────────────────────────────────────────────────────┐
...
| 2 | Executive Summary (includes Classification Breakdown as subsection) | `summary.overall_classification`, `summary.overall_confidence`, `test_report.summary`, `summary.by_classification` (sorted by count descending), `summary.cascading_hook_failures` (v3.2), `summary.blank_page_failures` (v3.2), `summary.data_assertion_failures` (v3.3), `summary.feature_area_health` (v3.3) |
| 3 | Build Information | `jenkins.job_name`, `jenkins.build_number`, `jenkins.build_result`, `jenkins.branch`, `jenkins.commit_sha` |
| 4 | Individual Test Failures | `per_test_analysis[]` (NOT `failed_tests`) — classification, confidence, error, reasoning, evidence, recommended fix, `failure_mode_category` (v3.3), `assertion_analysis` (v3.3), `root_cause_layer` + `root_cause_layer_name` + `investigation_steps_taken` + `cause_owner` (v3.8). Raises `ValueError` if `per_test_analysis` is missing or empty. |
| 5 | Common Patterns | `common_patterns[]` |
| 6 | Recommended Actions | `summary.priority_order[]` — sorted by priority |
| 7 | Cluster Investigation (v3.0) | `analysis_results.cluster_investigation` — degraded operators, pod diagnostics |
| 8 | Feature Knowledge & Cluster Investigation (v3.1) | `cluster_investigation_summary` (Tier 0 health, prerequisites, component health, playbook investigations), `cluster_access` status |
| 9 | Feature Area Grouping (v3.0) | `raw_data.feature_grounding.groups` — tests grouped by feature area with subsystem context |
| 10 | Environment Status | `environment.cluster_connectivity`, `environment.api_accessibility`, `environment.environment_score` |
| 11 | Feedback (v3.0) | CLI instructions for `feedback.py` — rate classifications for accuracy tracking |
| 12 | Footer | Framework version, data gathered timestamp |
...
```json
{
  ...
  "summary": {
    ...
    "feature_area_health": { "CLC": { "score": 1.0, "signal": "healthy" } },
    "priority_order": [...]
  },
  "per_test_breakdown": [
    {
      ...
      "investigation_steps_taken": ["Layer 12: Selector not in product source -> ROOT CAUSE"],
...
Generated by `src/reports/html_report.py` (called from `format_all()`). Self-contained interactive HTML report with no external dependencies.

**Features:**
- Classification breakdown with color-coded donut chart
- Per-test failure cards with expandable details
- Environment tab (maps `cluster-diagnosis.json` fields to visual health display via `_map_diagnosis_to_health()`, with fallback synthesis from simplified diagnosis format when structured fields are missing)
- Subsystem health, operator health, and managed cluster status
- Image integrity validation results
- Filterable by classification type

**Error handling:** If HTML generation fails, `format_all()` logs a warning and continues — the other three reports are still produced.

**Auto-open:** On macOS, `report.py` automatically opens the HTML report in the default browser after generation.
...
Cleaned up repos/ to save disk space (use --keep-repos to preserve)
  ...
  Open full report: file:///path/to/runs/acm-e2e_20260205_120000/analysis-report.html
