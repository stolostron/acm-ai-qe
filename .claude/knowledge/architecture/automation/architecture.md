...
|-----------|------|-----------|-----------|------|
| cluster-curator-controller | Hub deployment | multicluster-engine (MCE) | name=cluster-curator-controller | Orchestrates Ansible pre/post hooks for cluster lifecycle |
| AAP operator (external) | OLM subscription | openshift-operators | - | Ansible Automation Platform, provides AnsibleJob CRD |
...
- **Impact:** AnsibleJob CRD doesn't exist; hook execution fails.
  Template dropdown empty because there's no Tower to query.
...
### ansibletower.ts returns empty results
- **Impact:** Console proxy intercepts Tower API calls and returns
  `{count:0, results:[]}` without contacting Tower
- **Scope:** Console automation UI
- **Symptom:** Template dropdown appears empty even when Tower is healthy

### ClusterCurator SSE events dropped
- **Impact:** events.ts filters out ClusterCurator events, automation
  status doesn't update in real-time
- **Scope:** Console automation UI
- **Symptom:** Automation status appears stale in the UI
