...
3 slash commands: `/analyze`, `/gather`, `/quick`
...
3 app-level skills: `/generate`, `/review`, `/batch`

## Skills (`skills/`)

17 portable skills organized by category under `skills/<category>/`. Each has a `SKILL.md` entry point (YAML frontmatter + instructions). Shared skills are stateless tools; orchestrators compose them into pipelines.

| Category | Path | Skills | Orchestrators |
|----------|------|--------|---------------|
