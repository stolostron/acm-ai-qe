...
|-----------|------|-----------|-----------|------|
| grc-policy-propagator | Hub deployment (2 replicas) | MCH namespace (ocm) | app=grc-policy-propagator | Central hub controller: distributes policies, aggregates compliance |
| grc-policy-addon-controller | Hub deployment (2 replicas) | MCH namespace (ocm) | app=grc-policy-addon-controller | Manages GRC addons on spoke clusters |
...
| Dependency | Why |
|---|---|
| Infrastructure (klusterlet) | If klusterlet disconnected, compliance status stops updating |
