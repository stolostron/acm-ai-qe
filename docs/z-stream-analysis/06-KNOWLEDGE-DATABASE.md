...
**Canonical location:** `.claude/knowledge/` (unified database). Skills (`.claude/skills/acm-z-stream-analyzer/`, `acm-failure-classifier/`, `acm-data-enricher/`) read from `.claude/knowledge/` via `KNOWLEDGE_DIR`. Z-stream-specific content lives in `baselines/`, `failures/`, and `diagnostics/` within the unified database.
