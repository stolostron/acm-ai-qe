...
KNOWLEDGE_DIR = `${CLAUDE_SKILL_DIR}/../../../.claude/knowledge/`

**Claude Code:** With the skill under `ai_systems_v2/skills/hub-health/acm-hub-health-check/`, `../../../.claude/knowledge` is `ai_systems_v2/.claude/knowledge/` in the clone.

**Cursor (optional mirror only):** If you copy or symlink this skill under `~/.cursor/skills/` instead of loading it from the clone, also follow **`CURSOR-SYMLINK-INTEGRATION.md`** in this `skills/` folder so `../../../.claude/knowledge` resolves. **Claude Code / repo-only use does not need this.**
