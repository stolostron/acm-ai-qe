...
| Invalid (strict mode) | Raises error, stops report generation |
...
│         (best-effort — proceeds even if invalid)        │
└───────────────────────┬─────────────────────────────────┘
                        │
                        ▼
┌─────────────────────────────────────────────────────────┐
...
| 4 | Individual Test Failures | `per_test_analysis[]` — classification, confidence, error, reasoning, evidence, recommended fix, `failure_mode_category` (v3.3), `assertion_analysis` (v3.3), `root_cause_layer` + `root_cause_layer_name` + `investigation_steps_taken` + `cause_owner` (v3.8) |
