...
> **Playwright as-built:** Specs live in `console-e2e/src/tests/cluster/` (not Cypress paths below). Fixture: `@fixtures/acm-test`. Project: `--project cluster`. Pages: `ClusterListPage`, `ClusterNodesPage`, `ClusterSetsPage` (last not wired in fixture yet). Domain service: `ObservabilityService` for GPU specs. See `framework/playwright-patterns.md`.
