...
**Two copies, same content:** The app pipeline reads from `apps/z-stream-analysis/knowledge/` (app-local). Portable skills (`skills/z-stream/acm-z-stream-analyzer/`, `acm-failure-classifier/`, `acm-data-enricher/`) read from `.claude/knowledge/` (unified database) via `KNOWLEDGE_DIR`. Z-stream-specific content lives in `baselines/`, `failures/`, and `diagnostics/` within the unified database. Changes to domain knowledge should be applied to both locations.
