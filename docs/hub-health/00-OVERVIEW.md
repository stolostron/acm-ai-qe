...
├── .mcp.json                           ← MCP server configuration (acm-source, neo4j-rhacm, acm-search) [generated by setup.sh]
...
| Decision | Rationale |
|----------|-----------|
| Minimal custom code | Core agent is pure Claude Code with no runtime dependencies beyond `oc` and `claude`. Uses cluster introspection (8 live metadata sources via `oc` commands) and three MCP servers (acm-source for source code search, neo4j-rhacm for dependency graph queries, acm-search for fleet-wide resource queries via the search database). Custom code: `.claude/hooks/agent_trace.py` (session tracing hook, stdlib only, runs automatically) and `knowledge/refresh.py` (optional, for updating YAML baselines from a live cluster; requires Python 3 + PyYAML). |
