...
4. **The `direction` field in selector timeline must be computed** -- `recent_selector_changes.direction` must be one of `removed_from_product`, `automation_ahead_of_product`, `null`. Never leave it empty or set it to the raw commit message. Compute it from the diff hunks.
